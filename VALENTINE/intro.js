...
let fullText = ["Happy Valentine", "My Love â¤ï¸"];
...
function getResponsiveFontSize() {
  ...
  const isMobileLandscape = viewport.width <= 1024 && viewport.height < viewport.width;
  return isMobileLandscape ? 64 : 100; // Larger on mobile landscape
}

function getResponsiveDotSpacing() {
  const viewport = getViewportSize();
  const isMobileLandscape = viewport.width <= 1024 && viewport.height < viewport.width;
  return isMobileLandscape ? 5 : 4;
}
...
function getResponsiveLineHeight() {
  ...
  const viewport = getViewportSize();
  const isMobileLandscape = viewport.width <= 1024 && viewport.height < viewport.width;
  return fontSize * (isMobileLandscape ? 1.4 : 1.2);
}

function getResponsiveShootInterval() {
  const viewport = getViewportSize();
  const isMobileLandscape = viewport.width <= 1024 && viewport.height < viewport.width;
  return isMobileLandscape ? 32 : 22;
}
...
function generateCharDots(char, x, y) {
  ...
  const dotSpacing = getResponsiveDotSpacing();
...
function startAnimation() {
  ...
  scheduleBearChange();
  shootIntervalId = setInterval(shootDot, getResponsiveShootInterval());
  animate();
}
